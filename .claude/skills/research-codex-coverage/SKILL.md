...
- **Hooks are TOML-configured** under `[hooks]` in `config.toml`. The event names from `HooksToml` are: `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `Stop`, `PreCompact`, `PostCompact`, `PermissionRequest`. Each event takes an array of `MatcherGroup` objects with a `command` handler.
